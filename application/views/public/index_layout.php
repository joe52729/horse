<?php 
  ...
  if(isset($view_lang)){
    ...
    if($browser_preset_lang=='zh-CN'){
      $this->session->set_userdata('lang',$browser_preseㄊt_lang);
      $this->lang->load('general_cn_lang', 'cn');
      $view_lang = $browser_preset_lang;
      $this->session->mark_as_temp('lang', 86400);//86400 一天
    }  
...
{
    // redirect($this->config->item('api_url')[$this->session->userdata('lang')]['user_center'].'/login?return_url='.base_url('index/login'));
}
?>
<!DOCTYPE html>
<html lang="en">
